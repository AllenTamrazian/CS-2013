...
import Lab12.BSTModified.TreeNode;

public class AVLTreeModified<E> extends BST<E> {
	  /** Create an empty AVL tree using a natural comparator*/
	  public AVLTreeModified() { // super() is implicitly called
	  }

	  /** Create a BST with a specified comparator */
	  public AVLTreeModified(java.util.Comparator<E> c) {
	    super(c);
	  }

	  /** Create an AVL tree from an array of objects */
	  public AVLTreeModified(E[] objects) {
	    super(objects);
	  }
	  ...
	  public int nodeSize(TreeNode<E> root) {
		    if (root == null) {
		        return 0;
		    } else {
	...
	public E find(int k, TreeNode<E> root) {
		...
		if(k<=nodeSize(root.left))
		{
			return find(k, root.left);
		}
		if(k==nodeSize(root.left)+1)
		{
			return root.element;
		}
		if(k>nodeSize(root.left)+1)
		{
			return find(k-nodeSize(root.left)-1, root.right);
		}
		return find(k, root);
	}
	public static void main(String[] args) {
	    AVLTreeModified<Double> tree = new AVLTreeModified<>();
